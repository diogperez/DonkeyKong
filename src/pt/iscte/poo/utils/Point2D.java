...
public class Point2D implements Serializable { // Added 23-Apr-2018
	...
	//Metodos auxiliares para verificar posicoes
	public boolean isAbove(Point2D position) {
		return this.equals(new Point2D(position.getX(), position.getY() - 1));
	}
	
	public boolean isOnLeft(Point2D position) {
		return this.equals(new Point2D(position.getX() - 1, position.getY()));
	}
	
	public boolean isOnRight(Point2D position) {
		return this.equals(new Point2D(position.getX() + 1, position.getY()));
	}
}
